...
class Upload {//类定义开始
    ...
    public $thumb   =  true;
    // 图库类包路径
    public $imageClassPath = 'Image.class.php';
    // 缩略图最大宽度
    public $thumbMaxWidth = "480px";
    // 缩略图最大高度
    public $thumbMaxHeight = "800px";
    // 缩略图前缀
    ...
    public function __construct($maxSize='',$allowExts='',$allowTypes='',$savePath='',$saveRule='') {
        $this->thumbPath = UPLOAD_PATH .'/Home/';
        $this->savePath = UPLOAD_PATH .'/Home/';
    ...
    private function save($file) {
        ...
        if(!move_uploaded_file($file['tmp_name'], $this->autoCharset($filename,'utf-8','gbk'))) {
            $this->error = 'Upload files to save the error!';
            return false;
        }
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                /* 修复  fee*/
                $thumbPath    =  $thumbPath . $this->getSubName($file) . '/';
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    $savethumbname[] = $this->getSubName($file).'/'.$thumbPrefix[$i].substr($realFilename,0,strrpos($realFilename, '.')).$thumbSuffix[$i].'.'.$file['extension'];
                    Image::thumb($filename,$thumbname,'',$thumbWidth[$i],$thumbHeight[$i],true);
                }
                $this->thumbname = $savethumbname;
    ...
    public function upload($savePath ='') {
        ...
        if(!is_dir($savePath)) {
            ...
            if(is_dir(base64_decode($savePath))) {
                ...
                // 尝试创建目录
                if(!mkdir($savePath)){
                    $this->error  =  'The upload directories '.$savePath.' is not writable';
                    return false;
                }
            }
        }else {
        ...
        foreach($_FILES as $key => $file) {
            ...
            /* if(!empty($file['name'])) {
                ...
                if(function_exists($this->hashType) && $this->thumbRemoveOrigin === false) {
                    $fun =  $this->hashType;
                    $file['hash']   =  $fun($this->autoCharset($file['savepath'].$file['savename'],'utf-8','gbk'));
                }
                //上传成功后保存文件信息，供其他地方调用
                unset($file['tmp_name'],$file['error']);
                $fileInfo[$key] = $file; //改造 Fee
                $isUpload   = true;
            } */
            //var_dump($file['name']);exit;
            //过滤空的上传图像
            if (!empty($file['name'])){
               ...
               if($value[$k]==''){
                ...
                foreach ($file['error'] as $key=>$value){
                   ...
                   $fileInfo[$key] =  $this->uploadOne($file);
                }
                else {
                    $fileInfo[$key] = $this->uploadOne($file);
                }
        ...
        /* if($isUpload) {
            ...
            $this->error  =  'Did not choose to upload file!';
            return false;
        } */
    }
    ...
    public function uploadOne($file,$savePath=''){
        ...
        if(!is_dir($savePath)) {
            // 尝试创建目录
            if(!mkdir($savePath)){
                $this->error  =  'The upload directories '.$savePath.' does not exist';
                return false;
            }
        }else {
            if(!is_writeable($savePath)) {
        ...
        if(!empty($file['name'])) {
            ...
            foreach ($fileArray as $key=>$file){
                ...
                //保存上传文件 
                if(!$this->save($file)) return false;
                $file['savethumbname'] = $this->thumbname;
                if(function_exists($this->hashType)  && $this->thumbRemoveOrigin === false) {
                    $fun =  $this->hashType;
                    $file['hash']   =  $fun($this->autoCharset($file['savepath'].$file['savename'],'utf-8','gbk'));
                }
    ...
    protected function error($errorNo) {
         switch($errorNo) {
            case 1:
                $this->error = 'The uploaded file exceeds the value of the option to limit the php.ini upload_max_filesize';
                break;
            case 2:
                $this->error = 'Upload file size exceeds the HTML form MAX_FILE_SIZE option value specified';
                break;
            case 3:
    ...
    private function check($file) {
        ...
        if(!$this->checkExt($file['extension'])) {
            $this->error ='Upload a file type not allowed!';
            return false;
        }

        //检查是否合法上传
        if(!$this->checkUpload($file['tmp_name'])) {
            $this->error = 'Illegal to upload files!';
            return false;
        }
        return true;
    }
