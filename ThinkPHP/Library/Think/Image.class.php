...
class Image {
     ...
     * @return void
      +----------------------------------------------------------
     */
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true, $backgroundcolor='#FFFFFF') {
        // 获取原图信息
   ...
   $bgcolor = imagecolorallocate($thumbImg 
   ...
   $NewImgX = 0; 
   $NewImgY = 0;  
   if ( ($maxWidth / $maxHeight - $srcWidth / $srcHeight) > 0 ) { 
    $NewImgX = ( $maxWidth - $width ) / 2; 
    $NewImgY = ( $maxHeight - $height ) / 2; 
   } elseif ( ($maxWidth / $maxHeight - $srcWidth / $srcHeight) < 0 ) { 
    $NewImgX = ( $maxWidth - $width ) / 2; 
    $NewImgY = ( $maxHeight - $height ) / 2; 
   } 
            // 复制图片
            if (function_exists("ImageCopyResampled"))
               imagecopyresampled($thumbImg, $srcImg, $NewImgX, $NewImgY, 0, 0, $width, $height, $srcWidth, $srcHeight);
            else
                imagecopyresized($thumbImg, $srcImg, $NewImgX, $NewImgY, 0, 0, $width, $height, $srcWidth, $srcHeight);
